 ...
 *
 * The download is derived from the running version: an unmodified release build (or official
 * release image; see `channel` in `version.ts`) fetches its extensions from the per-version
 * descriptor at `<baseUrl>/by-version/<version>.json`. The switch is offered only on release
 * builds, so it stays hidden on `main`/nightly builds. `GRIST_EXT_FULL_EDITION_BASE_URL` points the
 * derivation at a self-hosted mirror, or disables it (when empty) for air-gapped installs.
 *
...
/** Default base URL for the per-version full edition descriptors published by Grist Labs. */
 ...
 * `build_version_file` in `buildtools/build.sh`. Builds of `main`/nightly report `channel` "core".
 */
...
/**
 * Base URL for the per-version descriptors from which the download is derived. Read from
 ...
 * fetching the per-version descriptor at `<baseUrl>/by-version/<version>.json` over TLS. Throws if
...
async function resolveOverlay(): Promise<{ url: string; sha256: string }> {
  ...
  return await fetchDescriptor(`${base}/by-version/${version}.json`);
}

/**
 * Fetches and validates the per-version descriptor at `url`, returning the download URL and its
 * expected checksum. Throws on a failed request or a malformed descriptor.
 */
async function fetchDescriptor(url: string): Promise<{ url: string; sha256: string }> {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`full edition descriptor request failed (${res.status}) for ${url}`);
    }
    const body = await res.json() as { url?: unknown; sha256?: unknown };
    if (typeof body.url !== "string" || !body.url ||
      typeof body.sha256 !== "string" || !body.sha256) {
      throw new Error(`full edition descriptor is malformed: ${url}`);
    }
