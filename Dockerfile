...
# .git is not in the build context, so the build stamps version.ts from these args instead of git
# (see build_version_file in buildtools/build.sh). GRIST_BUILD_CHANNEL=release enables the full
# edition switch; the release workflow sets it, nightly builds leave it unset.
