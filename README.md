...
| GRIST_ALLOW_FULL_EDITION_UPGRADE | on the `grist-oss` image, whether admins may switch the installation to the Full (Enterprise) edition at runtime — it downloads a complete copy of the full edition and restarts. Baked to "true" on `grist-oss`; clear it (set to "false"/unset) to disable the feature, which hides the admin button and stops conversions even if the pref was set earlier. Not set on the `grist`/`grist-ee` images. |
| GRIST_FULL_EDITION_DIR | writable directory where the runtime full-edition copy is downloaded and run from (baked to `/persist/grist-full-edition` on `grist-oss`). Must be local to each server — do not share one directory across multiple servers. |
| GRIST_FULL_EDITION_REF / GRIST_FULL_EDITION_URL_AMD64 / GRIST_FULL_EDITION_URL_ARM64 / GRIST_FULL_EDITION_SHA256_AMD64 / GRIST_FULL_EDITION_SHA256_ARM64 | internal: the ref (on-disk stamp), per-arch download URL, and per-arch sha256 checksum of the full-edition package, baked into the `grist-oss` image by the build. Not intended to be set by operators. |
