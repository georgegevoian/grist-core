...
describe("bootstrapFullEdition", function() {
  ...
  describe("maybeManageFullEdition", function() {
    ...
    describe("download + install", function() {
      ...
      let base: string;
      // The descriptor body served for this test; null serves a 404.
      let descriptorBody: string | null;
      ...
      beforeEach(async function() {
        ...
        fileServer = http.createServer((req, res) => {
          if (req.url === `/by-version/${version}.json`) {
            if (descriptorBody === null) { res.writeHead(404); res.end(); return; }
            res.writeHead(200, { "content-type": "application/json" });
            res.end(descriptorBody);
        ...
        descriptorBody = JSON.stringify({ url: `${base}/payload.tar.gz`, sha256: tarballSha });
        await setUseExtFullEdition(true);
      });
      ...
      it("rejects a checksum mismatch: no install, stays on built-in edition", async function() {
        sandbox.stub(Deps, "installAttempts").value(1);
        sandbox.stub(Deps, "installRetryDelayMs").value(0);
        descriptorBody = JSON.stringify({ url: `${base}/payload.tar.gz`, sha256: "deadbeef" });
      ...
      it("stays on core when the descriptor is missing", async function() {
        sandbox.stub(Deps, "installAttempts").value(1);
        sandbox.stub(Deps, "installRetryDelayMs").value(0);
        descriptorBody = null;
      ...
      it("stays on core when the descriptor is malformed", async function() {
        sandbox.stub(Deps, "installAttempts").value(1);
        sandbox.stub(Deps, "installRetryDelayMs").value(0);
        descriptorBody = "{ not json";
