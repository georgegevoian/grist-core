...
# GRIST_BUILD_CHANNEL is set, e.g. in Docker where no .git is present). The full edition switch is
# offered only on release builds (see app/server/lib/bootstrapFullEdition.ts).
build_version_file() {
