...
#
# On the release channel it also publishes a per-version descriptor that points at the tarball,
# which is how the running grist-oss finds this download (by its own version):
...
# The running grist-oss derives the descriptor URL from its own version, so read it from the image.
...
if [[ "$DRY_RUN" == 1 ]]; then
  ...
  if [[ "$CHANNEL" == "release" ]]; then
    echo "+ Would publish descriptor by-version/${VERSION}.json"
  fi
  exit 0
fi
...
# On a release, publish a per-version descriptor from which the running grist-oss resolves this
# download (see fetchDescriptor in app/server/lib/bootstrapFullEdition.ts). Nightly builds skip it:
# their version matches the last release and must not clobber that release's descriptor.
if [[ "$CHANNEL" == "release" ]]; then
  descriptor="$work/${VERSION}.json"
  printf '{"url":"%s","sha256":"%s"}\n' "$URL" "$SHA" > "$descriptor"
  aws s3 cp "$descriptor" "s3://$BUCKET/$PREFIX/by-version/${VERSION}.json"
